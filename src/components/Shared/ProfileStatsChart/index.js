...
const ProfileStatsChart = (props) => {
        ...
        .then(r => {
          if (ac.signal.aborted === false) {
            if (r.data.result === 1) {
              ...
              let tt = r.data.history.map(t => { return [new Date(t.when).getTime(), (t.priceUSD * t.copy).toFixed(2)] });
              setPrices(t => [{ data: tt }]);
  ...
  return (
    <StatsChartContainer>
      {isEnableChart === true ? <Chart options={opt} series={prices} type="area" height={200} /> : <div className='loading'>Loading...</div>}
    </StatsChartContainer>
  )
}
