...
export const Offer = (props) => {
  ...
  const { itemLoaded, favorite, comments, creatorInfo, unlistedInfo, reloadSaleInfoToOffer, nftSalesOnPurchase } = useOfferInfo();
  ...
  const refreshOfferList = () => {
    invokeServer('get', `/api/offer?collectionAddress=${itemLoaded?.collectionAddress.toLowerCase()}&tokenId=${itemLoaded?.tokenId}`)
      .then(r => {
  ...
  useEffect(() => {
    itemLoaded.collectionAddress && setIsImage(itemLoaded?.video === true ? false : true);
              ...
              <div>
                ...
                <div className="nft-review">
                  ...
                  <IconLabelWrapper className="owned-by" onClick={() => setIsOwnedModal(true)}>
                    <UserListIcon />
                    <span>{itemLoaded?.holderCount} owner{itemLoaded?.holderCount <= 1 ? '' : 's'}</span>
                  </IconLabelWrapper>
                  <IconLabelWrapper>
