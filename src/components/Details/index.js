...
export const Details = (props) => {
  ...
  const { saleInfo, nftItem, myFavorite, creatorInfo, sellerBalance, nftSalesOnPurchase, commentsOnPurchase, reloadSaleInfoToTrade } = useSaleItem();
  const { approveToken, buySale, removeSale } = useContract();

  const { invokeServer, reloadSales, copyToClipboard, refreshPage } = useGlobal();
  const { toastSuccess, toastError, toastInfo, showLoading, hideLoading } = useToast();
  ...
  useEffect(() => {
    if (saleInfo?.start !== undefined) {
      if (saleInfo.duration > 0) {
        let endTime = new Date(saleInfo.start).getTime() / 1000 + saleInfo.duration;
      ...
      timer = setTimeout(() => {
        if (ac.signal.aborted === false) {
          ...
          if (endTime >= nowTime) {
            ...
            let minutes = Math.floor((timeRemaining - days * (24 * 3600) - hours * 3600) / 60);
            let seconds = Math.floor(timeRemaining - days * (24 * 3600) - hours * 3600 - minutes * 60);
            setTimeRemainingText(`${days} days - ${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`)
          }
        }
      }, 1000)
    }
  ...
  const handleBuy = async (saleToBuy) => {
    if (saleToBuy.payment > 0) {
      ...
      let tx = await approveToken({ copy: saleToBuy.copy, payment: saleToBuy.payment, price: saleToBuy.price, fee: saleToBuy.fee });
      if (tx === undefined) {
        hideLoading();
    ...
    showLoading(auth.loggedUserName + ` is buying ${saleToBuy.copy} NFTs from ${saleToBuy.sellerName}...`);
    let tx = await buySale({ user: saleToBuy.sellerName, copy: saleToBuy.copy, saleId: saleToBuy.saleId, payment: saleToBuy.payment, price: saleToBuy.price, fee: saleToBuy.fee });
  ...
  const handleRemoveSale = async (saleToRemove) => {
    showLoading(auth.loggedUserName + ` is removing his/her sale(${saleToRemove.saleId})...`);

    let tx = await removeSale(saleToRemove.saleId);
    if (tx !== undefined) {
      // console.log('----------------', tx.events.RemoveFromSale.returnValues);
      await invokeServer('post', '/api/sale/remove', tx.events.RemoveFromSale.returnValues)
        .then(r => {
          if (r.data.result === 1) {
    ...
    <DetailsContainer>
      <div className="item-container">
        <div className="media-container">
          {isImage ? (
            ...
            <div className="media">
              <video src={nftItem?.image} preload="auto" autoPlay controls loop muted ref={videoRef}></video>
            </div>
          )}
        </div>
        <div className="detail-container">
              ...
              <div>
                ...
                <div className="nft-review">
                  ...
                  <IconLabelWrapper className="owned-by" onClick={() => setIsOwnedModal(true)}>
                    <UserListIcon />
                    <span>{nftItem?.holderCount} owner{nftItem?.holderCount <= 1 ? '' : 's'}</span>
                  </IconLabelWrapper>
                  <IconLabelWrapper>
                    {myFavorite && (myFavorite.set === true ? (
                      <SvgWrapper
                  ...
                  <div className="button-group">
                    {
                      saleInfo?.seller.toLowerCase() !== wallet.address.toLowerCase() ?
                        ...
                        <>
                          <div className='nft-owner'>You are the seller of this sale</div>
                          {
                            isEndedSale === true && (
                              <GradientButton
                    ...
                    <OutlineBox>
                      {
                        category === 'buy' ? `${saleInfo?.copy}/${sellerBalance}` :
                          category === 'bid' ? `${saleInfo?.copy}/${sellerBalance}` : ''
                      }
                    </OutlineBox>
                  }
          ...
          <CommentContainer>
            <CommentTab category={category} types={selectTypes(category)} active={activeTab} setActive={setActiveTab} sales={nftSalesOnPurchase} saleInfo={saleInfo} nftItem={nftItem} comments={commentsOnPurchase} refresh={reloadSaleInfoToTrade} />
          </CommentContainer>
        </div>
      </div>
      ...
      {isOwnedModal && (
        <MainModal
          ...
          <OwnedBy collection={nftItem?.collectionAddress} tokenId={nftItem?.tokenId} />
        </MainModal>
      )}
